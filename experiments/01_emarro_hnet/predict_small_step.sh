...
# =================================================================
# Usage: ./predict_small_step.sh
# Uses smaller stride (window_size // 4) for finer-grained predictions
# =================================================================

# 1. Configuration
# ----------------
INPUT_FILE="/workdir/zl843/GeneCAD/fine-tuning/input_file/Zmays/Zmays_833_Zm-B73-REFERENCE-NAM-5.0_chr1.fa"
OUTPUT_DIR="/workdir/zl843/GeneCAD/genecad_result/prediction_emarro_hnet_zmays_chr1_small_step"
SPECIES_ID="Zmays"
...
HEAD_MODEL="/workdir/zl843/GeneCAD/genecad_result/training_emarro/training_output/checkpoints/last.ckpt"
...
echo "================================================================="
echo "Running GeneCAD Prediction Pipeline (Small Step)"
echo "Input: $INPUT_FILE"
echo "Output: $OUTPUT_DIR"
echo "Base Model: $BASE_MODEL"
echo "Stride: window_size // 4 (2048)"
echo "================================================================="
...
# 3. Predict Tokens (using predict_small_step.py with stride = window_size // 4)
# -----------------
echo "[2/6] Generating token predictions (small step)..."
rm -rf "$PIPELINE_DIR/predictions.zarr"
$PYTHON scripts/predict_small_step.py create_predictions \
...
$PYTHON scripts/predict_small_step.py detect_intervals \
...
$PYTHON scripts/predict_small_step.py export_gff \
...
# Filter invalid genes (require UTRs = yes -> precision optimized)
...
# Filter invalid genes (require UTRs = no -> recall optimized)
$PYTHON scripts/gff.py filter_to_valid_genes \
    --input "$PIPELINE_DIR/predictions__raw__feat_len_2__gene_len_30.gff" \
...
# 7. Final Output
# ---------------
cp "$PIPELINE_DIR/predictions_precision__raw.gff" "$OUTPUT_DIR/predictions_precision.gff"
cp "$PIPELINE_DIR/predictions_recall__raw.gff" "$OUTPUT_DIR/predictions_recall.gff"

echo "================================================================="
echo "Done! Final predictions saved to:"
echo "Precision Optimized: $OUTPUT_DIR/predictions_precision.gff"
echo "Recall Optimized:    $OUTPUT_DIR/predictions_recall.gff"
echo "================================================================="
