...
def extract_fasta_sequences(input_paths: list[str], species_ids: list[str], output_path: str, chunk_size: int = DEFAULT_SEQUENCE_CHUNK_SIZE, tokenizer_path: str | None = None) -> None:
    ...
    for fasta_file, species_id in zip(input_paths, species_ids):
        ...
        with open_func(fasta_file, mode) as file:
            for record in SeqIO.parse(file, "fasta"):
                chrom_id = record.id
                if not chrom_id.lower().startswith("chr"):
                    logger.info(f"  Skipping non-chromosome record: {species_id}/{chrom_id}")
                    continue
                sequence_records[(species_id, chrom_id)] = record
                logger.info(f"  Added {species_id}/{chrom_id}, length: {len(record.seq)}")
...
def main():
    ...
    if args.command == "extract_gff_features":
        ...
        extract_fasta_sequences(args.input, args.species_id, args.output, 
