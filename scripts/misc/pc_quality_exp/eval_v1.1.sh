...
# PC Quality Filter Experiment - Evaluation v1.1
# Model: v1.1 (Athaliana + Osativa)
# Species: jregia, pvulgaris
# Usage: ./eval_v1.1.sh {original|pc-filtered}
...
if [ $# -ne 1 ]; then
    ...
    echo "  original     - Use original (unfiltered) ground truth (v1.1.0)"
    echo "  pc-filtered  - Use PC-filtered ground truth (v1.1.1)"
    exit 1
fi
...
echo "Model: v1.1 (Athaliana + Osativa)"
...
MODEL_CHECKPOINT="$PIPE_DIR/sweep/sweep-v1.1__cfg_013__arch_all__frzn_yes__lr_1e-04/pc-genome-annot/v1.1/checkpoints/last.ckpt"
...
if [ ! -f "$MODEL_CHECKPOINT" ]; then
    ...
    echo "Please ensure v1.1 training has completed successfully"
    exit 1
fi
