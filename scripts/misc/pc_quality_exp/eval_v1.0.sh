...
# PC Quality Filter Experiment - Evaluation v1.0
# Model: v1.0 (Athaliana only)
# Species: jregia, pvulgaris
# Usage: ./eval_v1.0.sh {original|pc-filtered}
...
if [ $# -ne 1 ]; then
    ...
    echo "  original     - Use original (unfiltered) ground truth (v1.0.0)"
    echo "  pc-filtered  - Use PC-filtered ground truth (v1.0.1)"
    exit 1
fi
...
echo "Model: v1.0 (Athaliana only)"
...
MODEL_CHECKPOINT="$PIPE_DIR/sweep/sweep-v1.0__cfg_013__arch_all__frzn_yes__lr_1e-04/pc-genome-annot/v1.0/checkpoints/last.ckpt"
...
if [ ! -f "$MODEL_CHECKPOINT" ]; then
    ...
    echo "Please ensure v1.0 training has completed successfully"
    exit 1
fi
