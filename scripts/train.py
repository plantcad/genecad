...
def parse_args(args: Optional[list[str]] = None) -> Args:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to prepared training dataset (zarr)",
    )
    parser.add_argument(
        "--val-dataset",
        type=str,
        required=True,
        help="Path to prepared validation dataset (zarr)",
    )
    parser.add_argument(
        "--output-dir", type=str, required=True, help="Directory for saving outputs"
    )
    parser.add_argument(
        "--learning-rate", type=float, default=8e-4, help="learning rate for the model"
    )
    parser.add_argument(
        "--learning-rate-decay",
        type=str,
        default="none",
        help='Whether to use learning rate decay; either "none" or "cosine"',
    )
    parser.add_argument(
        "--window-size",
        type=int,
        default=WINDOW_SIZE,
        help="Size of the window for processing sequences",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Type of checkpoint to resume training from; one of "model", "trainer"',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="batches for dataloader to prefetch",
    )
    parser.add_argument(
        "--project-name", default="test-project", help="wandb project name"
    )
    parser.add_argument("--run-name", default="test-run", help="wandb run name")
    parser.add_argument(
        "--run-id",
        default=None,
        help="wandb run id to be resumed, if using a checkpoint",
    )
    ...
    parser.add_argument(
        ...
        choices=["encoder-only", "sequence-only", "classifier-only", "all"],
    ...
    parser.add_argument(
        ...
        default="yes",
        choices=["yes", "no"],
        help="Freeze the base encoder during training",
    )
    parser.add_argument(
        "--torch-compile",
        type=str,
...
def sample_transform(ds: xr.Dataset) -> dict:
    ...
    if "inputs_embeds" in sample_dict:
        sample_dict["inputs_embeds"] = torch.tensor(
            sample_dict["inputs_embeds"], dtype=torch.bfloat16
        )
    return sample_dict
...
def train(args: Args) -> None:
    ...
    logger.info(
        f"Training dataset loaded with {len(train_dataset)} samples ({len(train_dataset.datasets)} datasets)"
    )
    ...
    # Setup WandB logging
    logger.info(
        f"Setting up WandB logger with project={args.project_name}, run={args.run_name}, id={args.run_id}"
    )
