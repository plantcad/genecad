...
def prepare_command(args: Args) -> None:
    ...
    for strand in strands:
        ...
        if len(feature_logits.sequence) > min_length:
            logger.info(f"Truncating feature logits from {len(feature_logits.sequence)} to {min_length}")
            feature_logits = feature_logits.isel(sequence=slice(min_length))
        if len(region_labels.sequence) > min_length:
            logger.info(f"Truncating region labels from {len(region_labels.sequence)} to {min_length}")
            region_labels = region_labels.isel(sequence=slice(min_length))
        
        # Validate coordinates match
        validate_coordinates(
            feature_logits.sequence.values, 
            region_labels.sequence.values
        )
        ...
        start_indices = list(range(0, sequence_length - args.window_size + 1, args.window_stride))
        
        # Shuffle indices if requested
        if args.shuffle == "yes":
            rs = np.random.RandomState(args.seed)
            start_indices = rs.permutation(start_indices).tolist()
            logger.info(f"Shuffled {len(start_indices)} start indices with seed {args.seed}")
        else:
            logger.info(f"Using sequential order for {len(start_indices)} start indices")
        ...
        logger.info(f"Split {len(start_indices)} indices into {len(train_indices)} train and {len(val_indices)} validation")
        ...
        for split_name, split_indices in [("train", train_indices), ("valid", val_indices)]:
            ...
            for i, start_idx in enumerate(split_indices):
                ...
                if args.window_limit is not None and (i+1) >= args.window_limit:
                    logger.info(f"Reached window limit of {args.window_limit} for split {split_name}, stopping window creation")
                    break
                    
                # Extract window data using the same start_idx regardless of strand
                window_logits = feature_logits_array[start_idx:(start_idx + args.window_size)]
                window_labels = feature_labels_array[start_idx:(start_idx + args.window_size)]
                ...
                if len(window_logits) != args.window_size:
                    logger.warning(f"Skipping window {start_idx} due to incorrect size: {len(window_logits)} != {args.window_size}")
                    continue
            ...
            logger.info(f"Created {num_windows} {split_name} windows for {args.species_id}:{args.chromosome_id}:{strand}")
            ...
            if strand_windows:
                ...
                species_ids = np.array([w['species_id'] for w in strand_windows], dtype='<U64')
                chromosome_ids = np.array([w['chromosome_id'] for w in strand_windows], dtype='<U64')
                ...
                strand_ds = xr.Dataset(
                    data_vars={
                        'input_logits': (['sample', 'sequence', 'feature'], input_logits),
                        'target_labels': (['sample', 'sequence', 'feature'], target_labels),
                ...
                strand_ds.to_zarr(
                    split_output,
                    zarr_format=2,
                    **(dict(append_dim="sample") if os.path.exists(split_output) else {}),
                    consolidated=True
                )
                
                total_windows += len(strand_windows)
                logger.info(f"Saved {len(strand_windows)} {split_name} windows for {args.species_id}:{args.chromosome_id}:{strand} to {split_output}")
    
    if total_windows == 0:
        raise ValueError("No valid training windows were created")
    
    logger.info(f"Data preparation complete. Created {total_windows} total training samples.")
...
def train_command(args: Args) -> None:
    ...
    def sample_transform(ds: xr.Dataset) -> dict:
        ...
        if ds.input_logits.dims != ('sequence', 'feature'):
            raise AssertionError(f"Expected ('sequence', 'feature') dims for input_logits, got {ds.input_logits.dims}")
        if ds.target_labels.dims != ('sequence', 'feature'):
            raise AssertionError(f"Expected ('sequence', 'feature') dims for target_labels, got {ds.target_labels.dims}")
    ...
    logger.info(f"Example training dataset (1 of {len(train_dataset.datasets)}): {train_dataset.datasets[0]}")
...
def apply_command(args: Args) -> None:
    ...
    for strand in strands:
        ...
        feature_logits = pred_ds.feature_logits.sel(strand=strand).transpose("sequence", "feature")
        sequence_length = feature_logits.sizes["sequence"]
        num_features = feature_logits.sizes["feature"]
        
        logger.info(f"Processing {sequence_length} positions with {num_features} features")
        ...
        with torch.inference_mode():
            for window_data, local_window, _ in windows:
                ...
                if input_data.shape != (args.window_size, num_features):
                    raise AssertionError(f"Expected shape ({args.window_size}, {num_features}), got {input_data.shape}")
                ...
                assert output_array.shape == (args.window_size, num_features), f"Expected shape ({args.window_size}, {num_features}), got {output_array.shape}"
        ...
        if refined_logits.shape != feature_logits_array.shape:
            raise AssertionError(f"Shape mismatch: expected {feature_logits_array.shape}, got {refined_logits.shape}")
        ...
        strand_ds["feature_predictions"] = strand_ds["feature_logits"].argmax(dim="feature")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="UNET-based refining pipeline for gene annotation predictions")
    subparsers = parser.add_subparsers(dest="command", required=True, help="Command to execute")
    
    # Prepare command
    prepare_parser = subparsers.add_parser("prepare", help="Create training datasets from predictions and labels")
    ...
    prepare_parser.add_argument('--prediction-dataset', required=True, help="Path to prediction dataset")
    prepare_parser.add_argument('--labels-dataset', required=True, help="Path to labels dataset")
    prepare_parser.add_argument('--output', required=True, help="Path to output training dataset")
    prepare_parser.add_argument('--window-size', type=int, default=1_048_576, help="Window size for chunking (default: 1,048,576)")
    prepare_parser.add_argument('--window-stride', type=int, default=524_288, help="Window stride for chunking (default: 524,288)")
    prepare_parser.add_argument('--window-limit', type=int, default=None, help="Maximum number of windows to process (default: None, process all windows). Useful for debugging.")
    prepare_parser.add_argument('--shuffle', type=str, choices=["yes", "no"], default="yes", help="Shuffle start indices (default: yes)")
    prepare_parser.add_argument('--seed', type=int, default=42, help="Random seed (default: 42)")
    prepare_parser.add_argument('--val-proportion', type=float, default=0.05, help="Proportion of windows to use for validation (default: 0.05)")
    
    # Train command
    train_parser = subparsers.add_parser("train", help="Train UNET model for refining")
    train_parser.add_argument('--train-dataset', required=True, help="Path to training dataset")
    train_parser.add_argument('--val-dataset', required=True, help="Path to validation dataset")
    train_parser.add_argument('--output-dir', required=True, help="Directory for model outputs")
    ...
    train_parser.add_argument('--log-frequency', type=int, default=5, help="Frequency of logging during training (default: 5)")
    train_parser.add_argument('--checkpoint-frequency', type=int, default=100, help="Number of steps between saving checkpoints (default: 100)")
    train_parser.add_argument('--project-name', default='test', help="W&B project name (default: test)")
    train_parser.add_argument('--run-name', default='test', help="W&B run name (default: test)")
    train_parser.add_argument('--seed', type=int, default=42, help="Random seed (default: 42)")
    
    # Apply command
    apply_parser = subparsers.add_parser("apply", help="Apply trained model to refine predictions")
    apply_parser.add_argument('--input', required=True, help="Path to prediction dataset")
    apply_parser.add_argument('--output', required=True, help="Path to output refined dataset")
    apply_parser.add_argument('--model-checkpoint', required=True, help="Path to trained model checkpoint")
    apply_parser.add_argument('--window-size', type=int, default=1_048_576, help="Window size for inference (default: 1,048,576)")
    apply_parser.add_argument('--window-stride', type=int, default=524_288, help="Window stride for inference (default: 524,288)")
