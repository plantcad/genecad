...
def _generate_training_windows(task):
    ...
    logger.info(f"[{species_id}/{chrom_id}] Processing {task_id}/{total_tasks}: Loading dataset")
    ...
    for strand_name in ds.strand.values:
        ...
        for w_start, w_end in selected_windows:
            ...
            window_feature_labels = window_labels.feature_labels.argmax(dim="feature").values
    ...
    if dataset_samples:
        ...
        logger.info(f"[{species_id}/{chrom_id}] Creating dataset with {len(dataset_samples)} samples")
        data_vars = {}
        for key in dataset_samples[0].keys():
            ...
            if values.ndim == 1:
                ...
                raise ValueError(f"Unexpected number of dimensions for key '{key}': {values.ndim}. Expected 1 or 2 dimensions.")
        ...
        ds_out.to_zarr(output_path, group=output_group, zarr_format=2, consolidated=True, mode="w")
    ...
    if local_windows_processed > 0:
        ...
        logger.info(f"[{species_id}/{chrom_id}]   Windows processed: {local_windows_processed}")
        logger.info(f"[{species_id}/{chrom_id}]   Windows kept: {local_windows_kept} ({kept_pct:.1f}%)")
        logger.info(f"[{species_id}/{chrom_id}]   Windows skipped: {local_windows_skipped} ({skipped_pct:.1f}%)")
        logger.info(f"[{species_id}/{chrom_id}]   Samples generated: {len(dataset_samples)}")
    else:
...
def summarize_window_stats(all_stats: list[dict]) -> None:
    ...
    for stats in all_stats:
        logger.info(f"  {stats['species_id']}/{stats['chrom_id']}: {stats['windows_kept']} kept, {stats['windows_skipped']} skipped")
    ...
    # Counts table
    counts_table = df.pivot(index='Dataset', columns='Tag', values='Count').fillna(0).astype(int)
    logger.info(f"Tag Counts:\n{counts_table}")
    
    # Percentages table  
    freq_table = df.pivot(index='Dataset', columns='Tag', values='Frequency').fillna(0.0)
...
def generate_training_splits(args: Args) -> None:
    ...
    for i, ds in enumerate(datasets):
        ...
        logger.info(f"[{species_id}/{chrom_id}] Processing dataset {i+1}/{len(datasets)}")
        ...
        logger.info(f"  [{species_id}/{chrom_id}] Total samples: {n_samples}, Train: {len(splits[0][0])}, Valid: {len(splits[1][0])}")
        ...
        for split_indices, output_path, split_name in splits:
            if len(split_indices) > 0:
                ...
                split_ds = ds.isel(sample=split_indices).assign(
                    sample_index=('sample', np.arange(start_idx, start_idx + len(split_indices)))
                )
                
                # Drop all attrs then only keep the specified ones
                split_ds = split_ds.drop_attrs()
                split_ds.attrs.update({k: ds.attrs[k] for k in ['feature_labels_classes', 'label_classes'] if k in ds.attrs})
    ...
    for output_path, split_name in [(args.train_output, "train"), (args.valid_output, "valid")]:
...
def main():
    parser = argparse.ArgumentParser(description="Gene Annotation Training Window Generation")
    subparsers = parser.add_subparsers(dest="command", required=True, help="Command to execute")
    
    # Generate training windows command
    windows_parser = subparsers.add_parser("generate_training_windows", help="Generate training windows from sequence datasets")
    windows_parser.add_argument('--input', required=True, help='Path to input Zarr dataset with sequence data')
    windows_parser.add_argument('--output', required=True, help='Path to output Zarr dataset')
    windows_parser.add_argument('--window-size', type=int, default=8192, help='Size of training windows')
    windows_parser.add_argument('--mask-prop', type=float, default=0.8, help='Maximum proportion of masked sites allowed')
    windows_parser.add_argument('--intergenic-proportion', type=float, default=0.8, help='Proportion of intergenic windows relative to total windows (e.g., 0.5 = equal numbers of intergenic and genic windows, 0.8 = 8 intergenic per 2 genic windows)')
    windows_parser.add_argument('--intergenic-threshold', type=float, default=0.99, help='Threshold for classifying windows as intergenic (windows with intergenic proportion > threshold are considered intergenic)')
    windows_parser.add_argument('--chunk-size', type=int, default=2048, help='Chunk size for sample dimension')
    windows_parser.add_argument('--seed', type=int, default=42, help='Random seed for window selection')
    windows_parser.add_argument('--num-workers', type=int, default=None, help='Number of processes to use for parallel processing (each worker uses ~5-6G RAM; default: sequential processing)')
    
    # Generate training splits command
    splits_parser = subparsers.add_parser("generate_training_splits", help="Generate training splits from windowed datasets")
    splits_parser.add_argument('--input', required=True, help='Path to input windowed Zarr dataset')
    splits_parser.add_argument('--train-output', required=True, help='Path to training output Zarr dataset')
    splits_parser.add_argument('--valid-output', required=True, help='Path to validation output Zarr dataset')
    splits_parser.add_argument('--valid-proportion', type=float, default=0.2, help='Proportion of samples for validation')
    splits_parser.add_argument('--seed', type=int, default=42, help='Random seed for splitting')
    splits_parser.add_argument('--chunk-size', type=int, default=32, help='Chunk size for sample dimension; align this to desired chunk sizes for training dataloader')
