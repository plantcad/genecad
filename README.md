...
> [!NOTE] At the current time, the training pipeline is not set up to add arbitrary new species for fine-tuning
...
> [!WARNING] While training can be performed on a single NVIDIA GPU, it is very slow. It is strongly recommended
...
7. [Refine with ReelProtein](docs/refine.md) - `scripts/refine.py` - **GPU REQUIRED** - Uses ReelProtein to evaluate  and filter gene models for likely
