#!/bin/bash
# Fine-tune GeneCAD classifier on Emarro HNet base encoder — Multispecies
# Usage: ./train_multispecies.sh
set -euo pipefail
...
RAW_FASTA_DIR="/workdir/plantcad_architecture_tests/fasta_softmasked"
OLD_RAW_FASTA_DIR="/workdir/plantcad_architecture_tests/training_data/fasta/raw_fastas"
SPECIES_IDS="Athaliana Osativa Gmax Hvulgare Ptrichocarpa Ncolorata Ckanehirae Acoerulea Dcarota Zmarina"

# Pipeline directories
WORK_DIR="/workdir/zl843/GeneCAD/genecad_result"
PIPELINE_DIR="$WORK_DIR/training_emarro_multispecies_10genomes"
...
ATHALIANA_FASTA="$OLD_RAW_FASTA_DIR/Athaliana_447_TAIR10.fa"
OSATIVA_GFF="$RAW_GFF_DIR/Osativa_top_transcript.gff3"
OSATIVA_FASTA="$OLD_RAW_FASTA_DIR/Osativa_323_v7.0.fa"
GMAX_GFF="$RAW_GFF_DIR/Gmax_top_transcript.gff3"
GMAX_FASTA="/workdir/zl843/GeneCAD/fine-tuning/input_file/soybean/Phytozome/PhytozomeV13/Gmax/Wm82.a6.v1/assembly/Gmax_880_v6.0.fa"
HVULGARE_GFF="$RAW_GFF_DIR/Hvulgare_top_transcript.gff3"
HVULGARE_FASTA_GZ="/workdir/zl843/GeneCAD/reelAnno/phytozome/Phytozome/PhytozomeV12/early_release/Hvulgare_462_r1/assembly/Hvulgare_462_r1.fa.gz"
PTRICHOCARPA_GFF="$RAW_GFF_DIR/Ptrichocarpa_top_transcript.gff3"
PTRICHOCARPA_FASTA_GZ="/workdir/zl843/GeneCAD/reelAnno/phytozome/Phytozome/PhytozomeV13/Ptrichocarpa/v4.1/assembly/Ptrichocarpa_533_v4.0.fa.gz"

for f in "$ATHALIANA_GFF" "$ATHALIANA_FASTA" "$OSATIVA_GFF" "$OSATIVA_FASTA" "$GMAX_GFF" "$GMAX_FASTA" "$HVULGARE_GFF" "$HVULGARE_FASTA_GZ" "$PTRICHOCARPA_GFF" "$PTRICHOCARPA_FASTA_GZ"; do
    if [ ! -f "$f" ]; then
...
ln -sf "$GMAX_GFF" "$DATA_DIR/gff/Gmax_880_Wm82.a6.v1.gene.gff3" 2>/dev/null || true
# Compress GMAX_FASTA to match the .fa.gz config expectation natively
echo "  Compressing GMAX_FASTA to match config expectation..."
if [ ! -f "$DATA_DIR/fasta/Gmax_880_v6.0.fa.gz" ]; then
    gzip -c "$GMAX_FASTA" > "$DATA_DIR/fasta/Gmax_880_v6.0.fa.gz"
fi

ln -sf "$HVULGARE_GFF" "$DATA_DIR/gff/HvulgareMorex_702_V3.gene.gff3" 2>/dev/null || true
ln -sf "$HVULGARE_FASTA_GZ" "$DATA_DIR/fasta/HvulgareMorex_702_V3.fa.gz" 2>/dev/null || true

ln -sf "$PTRICHOCARPA_GFF" "$DATA_DIR/gff/Ptrichocarpa_533_v4.1.gene.gff3" 2>/dev/null || true
ln -sf "$PTRICHOCARPA_FASTA_GZ" "$DATA_DIR/fasta/Ptrichocarpa_533_v4.0.fa.gz" 2>/dev/null || true
...
echo "  GFF:   $GMAX_GFF -> $DATA_DIR/gff/Gmax_880_Wm82.a6.v1.gene.gff3"
echo "  FASTA: $GMAX_FASTA -> $DATA_DIR/fasta/Gmax_880_v6.0.fa.gz"
echo "  GFF:   $HVULGARE_GFF -> $DATA_DIR/gff/HvulgareMorex_702_V3.gene.gff3"
echo "  FASTA: $HVULGARE_FASTA_GZ -> $DATA_DIR/fasta/HvulgareMorex_702_V3.fa.gz"
echo "  GFF:   $PTRICHOCARPA_GFF -> $DATA_DIR/gff/Ptrichocarpa_533_v4.1.gene.gff3"
echo "  FASTA: $PTRICHOCARPA_FASTA_GZ -> $DATA_DIR/fasta/Ptrichocarpa_533_v4.0.fa.gz"

# ==================== Link New Target Species ====================
NEW_SPECIES="Ncolorata Ckanehirae Acoerulea Dcarota Zmarina"
