...
for strand in ["positive", "negative"]:
    ...
    for store in sorted(base.glob("predictions.*.zarr")):
        ...
        for i in range((m["shape"][0] + step - 1) // step):
            # Deliberately wrong control: positional (chunk-order) coordinates
            # rather than the cache's own stored coordinates.
            coords = np.arange(
                offset + i * step, offset + min(m["shape"][0], (i + 1) * step)
            )
            needed = []
            for r in relevant:
                lo, hi = int(r["window_start"]), int(r["window_end"])
