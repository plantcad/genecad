...
def attempt_visualization(fn):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            import traceback
            logger.warning(f"Non-critical visualization error occurred in {fn.__name__}: {e}")
...
def visualize_token_predictions(
    ...
    im = ax1.imshow(example_logits.T, aspect='auto', cmap='RdYlGn', interpolation="none")
    ...
    ax1.plot(x_coords, y_coords, '-', color='black', alpha=0.7, linewidth=1.5, label='Label sequence')
    
    # Plot scatter points over the line
    ax1.scatter(x_coords, y_coords, marker='s', s=20, edgecolor='black', facecolor='none', label='True labels')
    ...
    ax1.set_title(f'Sequence Logits and Labels (Step {module.global_step}, Sample {sample_name})')
    ...
    true_class_logits[example_mask] = example_logits[x_positions[example_mask], example_labels[example_mask].astype(int)]
    
    # Plot the true class logits with a continuous line
    ax2.plot(x_positions, true_class_logits, '-', color='black', alpha=0.7, linewidth=.5, label='True class logits')
    
    # Add red dots at positions where labels change
    label_change_positions = np.nonzero(np.diff(example_labels))[0] + 1
    ax2.scatter(
        label_change_positions, true_class_logits[label_change_positions], 
        color='#d62728', s=50, zorder=3, label='Label changes'
    )
    ...
    path = get_visualization_path(prefix, "token_prediction", module.global_step, batch_idx)
...
def visualize_entity_predictions(
    ...
    im0 = ax0.imshow(example_logits.T, aspect='auto', cmap='RdYlGn', interpolation="none")
    ...
    # Add title
    fig.suptitle(f'Entity Predictions (Step {module.global_step}, Sample {sample_name})', fontsize=16)
    
    # Adjust layout
    plt.tight_layout(rect=[0, 0.08, 1, 0.95])
    
    # Save and log the figure
    path = get_visualization_path(prefix, "entity_prediction", module.global_step, batch_idx)
...
def visualize_entity_intervals(
    ...
    sample_index, sample_name = _resolve_sample_info(
        sample_index=sample_index,
        sample_name=sample_name,
        default_index=lambda: int(entity_intervals['sample_index'].value_counts().idxmax()),
    )
    
    # Filter intervals for the selected sample
    sample_intervals = entity_intervals[entity_intervals['sample_index'] == sample_index]
    ...
    gs = GridSpec(len(unique_entities) + 1, 1, height_ratios=[2] * len(unique_entities) + [0.5])
    ...
    for i, entity in enumerate(unique_entities):
        ...
        for _, row in entity_data[entity_data['labeled']].iterrows():
            ax.axhspan(labeled_y - 0.4, labeled_y + 0.4, xmin=row['start']/sequence_length, 
                       xmax=(row['stop'] + 1)/sequence_length, alpha=0.6, color=cmap(entity))
            has_labeled = True
        ...
        for _, row in entity_data[entity_data['predicted']].iterrows():
            if row['labeled']:
                ...
                ax.axhspan(predicted_y - 0.4, predicted_y + 0.4, xmin=row['start']/sequence_length, 
                          xmax=(row['stop'] + 1)/sequence_length, alpha=0.6, color=correct_color)
                has_correct = True
            else:
                # Incorrect prediction
                ax.axhspan(predicted_y - 0.4, predicted_y + 0.4, xmin=row['start']/sequence_length, 
                          xmax=(row['stop'] + 1)/sequence_length, alpha=0.6, color=incorrect_color)
                has_incorrect = True
        ...
        if has_incorrect:
            handles.append(plt.Rectangle((0, 0), 1, 1, alpha=0.6, color=incorrect_color))
            labels.append('Incorrect Prediction')
        
        if handles:
            ax.legend(handles, labels, loc='upper right', framealpha=0.5, fancybox=True, shadow=True)
    ...
    mask_ax.imshow(example_mask.reshape(1, -1), aspect='auto', cmap='binary', vmin=0, vmax=1, interpolation="none")
    ...
    fig.suptitle(f'Entity Intervals Visualization (Step {module.global_step}, Sample {sample_name})', fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    
    # Save and log the figure
    path = get_visualization_path(prefix, "entity_interval", module.global_step, batch_idx)
...
def visualize_entity_interval_performance(
    ...
    all_metrics = pd.concat([overall_metrics, sample_metrics], axis=0, ignore_index=True)
    
    all_metrics = all_metrics[[
        'sample_index', 'entity', 'entity_name', 'true_intervals', 
        'predicted_intervals', 'true_positives', 'precision', 'recall', 'f1'
    ]]
    
    # Highlight the "All Samples" rows
    def highlight_all_samples(row):
        return ['font-weight: bold; border: 2px solid #4682B4' if row['sample_index'] == 'All Samples' else '' for _ in row]
    ...
    styled_table = (
        ...
        .background_gradient(cmap='Blues', subset=['true_intervals', 'predicted_intervals', 'true_positives'])
        .set_table_attributes('class="table table-bordered table-hover"')
        .set_caption(caption)
    )
    
    # Save table to HTML file
    path = get_visualization_path(prefix, "entity_interval_performance", module.global_step, batch_idx, extension='html')
    logger.info(f"Saving entity performance table to {path}")
    
    with open(path, 'w') as f:
        f.write('<html><head><style>body{font-family:Arial,sans-serif;}' +
