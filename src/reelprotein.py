...
def extract_candidate_proteins(genes, genome_fasta):
    ...
    logger.info(f"[Step 1] Found {len(protein_candidates)} candidate sequences (single + merged).")
...
def score_proteins(features_df, model_source):
    ...
    """
    logger.info(f"[Step 3] Downloading/Loading models from Hugging Face: {model_source}")
    
    try:
        # Always use snapshot_download. It handles caching automatically.
        model_dir = snapshot_download(repo_id=model_source, allow_patterns=["*.json"], repo_type="model")
    except Exception as e:
        raise RuntimeError(f"Failed to download models from Hugging Face ({model_source}): {e}") from e

    model_files = sorted(glob.glob(os.path.join(model_dir, '*.json')))
    
    if not model_files:
        raise FileNotFoundError(f"No .json models found in downloaded cache: {model_dir}")
