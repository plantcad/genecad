...
def convert_entity_intervals_to_labels(
    ...
    invalid_labels = intervals[
        (intervals["label"] < 1) | (intervals["label"] > num_labels)
    ]
    if not invalid_labels.empty:
        raise ValueError(
            f"Labels must be in [1, {num_labels}]. Invalid examples:\n{invalid_labels.head()}"
        )
    ...
    if not invalid_intervals.empty:
        raise ValueError(
            f"Intervals must be within domain {domain}. Invalid examples:\n{invalid_intervals.head()}"
        )
    ...
    result = _convert_entity_intervals_to_labels(
        starts, stops, labels, domain_start, result
    )
...
def _validate_labels(labels: np.ndarray) -> np.ndarray:
    ...
    if not np.issubdtype(labels.dtype, np.integer):
        raise ValueError(
            f"labels must be a numpy array of integers, got {labels.dtype=}"
        )
    if np.any(labels < -1):
        raise ValueError(
            f"labels must be in the set {{-1, 0, 1, 2, 3, ...}}, got {set(labels)=}"
        )
    return labels
...
def convert_biluo_index_to_class_name(
    label: int, entity_names: list[str], sentinel_names: tuple[str, str] | None = None
) -> str:
    sentinel_names = _default_sentinels(sentinel_names)
    entity_name = convert_biluo_index_to_entity_name(
        label, entity_names, sentinel_names
    )
    if label < 1:
        return entity_name
    else:
...
def _default_sentinels(
    sentinel_names: tuple[str, str] | None = None,
) -> tuple[str, str]:
    if sentinel_names is not None:
        if len(sentinel_names) != 2:
            raise ValueError(
                f"sentinel_names must be a tuple of length 2, got {sentinel_names=}"
            )
...
def convert_biluo_index_to_entity_name(
    label: int, entity_names: list[str], sentinel_names: tuple[str, str] | None = None
) -> str:
    sentinel_names = _default_sentinels(sentinel_names)
    if label < -1:
...
def convert_to_entity_labels(labels: np.ndarray) -> np.ndarray:
    ...
    entity_labels = np.where(
        np.isin(labels, [-1, 0]), labels, ((labels - 1) // N_BILUO_TAGS) + 1
    )
    return entity_labels


def convert_entity_labels_to_intervals(
    labels: np.ndarray, class_groups: list[list[int]], mask: np.ndarray | None = None
) -> pd.DataFrame:
...
def find_intervals(labels: np.ndarray, mask: np.ndarray | None = None) -> np.ndarray:
    ...
    if mask is not None:
        if mask.shape != labels.shape:
            raise ValueError(
                f"Mask must be the same shape as labels; got {mask.shape=} and {labels.shape=}"
            )
...
def find_group_intervals(
    ...
    if not np.issubdtype(input_classes.dtype, np.integer):
        raise ValueError(
            f"class_groups must contain only integer values, got {input_classes=}"
        )
    if (input_classes <= 0).any():
        raise ValueError(
            f"class_groups must contain only positive integers, got {input_classes=}"
        )
    ...
    if not np.all(np.isin(labels, np.r_[0, input_classes])):
        ...
        raise ValueError(
            f"labels must be in the set {{0, 1, 2, ..., {max_class}}}, got {set(labels)=}"
        )
...
def create_sequence_windows(
    ...
    for start, stop, v_start, v_stop in zip(*global_bounds, *local_bounds):
        ...
        local_window = (
            v_start - start,
            v_stop - start,
        )  # define local window relative to global window
...
def create_prediction_windows(
    ...
    if sequence_length % stride != 0:
        raise ValueError(
            f"sequence_length must be divisible by stride, got {sequence_length} and {stride} (quotient={sequence_length / stride})"
        )
    ...
    if len(v_starts) > 1:
        ...
        if np.any(gaps != 0):
            raise AssertionError(
                f"Valid regions have gaps or overlaps at positions: {np.where(gaps != 0)[0]}"
            )

    # Validate full coverage
    if len(v_starts) > 1 and (v_starts[0] != 0 or v_stops[-1] != sequence_length):
        raise AssertionError(
            f"Valid regions don't span full sequence: starts at {v_starts[0]}, ends at {v_stops[-1]}"
        )
...
def transition_matrix_stationary_distribution(
...
def _validate_decode_inputs(
    ...
    if transition_matrix.shape != (N, N):
        raise ValueError(
            f"Transition matrix shape {transition_matrix.shape} doesn't match emission probabilities states {N}"
        )
    ...
    if initial_probs is None:
        ...
        if initial_probs.shape != (N,):
            raise ValueError(
                f"Initial probabilities shape {initial_probs.shape} doesn't match number of states {N}"
            )
    ...
    while i < len(labels):
        ...
        if labels[i] == start_class:
            ...
            while j < len(labels) and (
                (j - start_pos <= max_length) or (max_length < 0)
            ):
...
def replace_interval_classes(
    ...
    if max_length is None:
        max_length = -1
    return _replace_interval_classes(
        labels,
        start_class,
        end_class,
        middle_class,
...
def regularize_transition_matrix(
    transition_matrix: npt.ArrayLike, alpha: float
) -> npt.ArrayLike:
...
def brute_force_decode(
    ...
    for path in paths:
        ...
        for t in range(1, T):
            prob *= (
                transition_matrix[path_array[t - 1], path_array[t]]
                * emission_probs[t, path_array[t]]
            )
