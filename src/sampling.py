...
def get_tag_stats(
    tag_labels: xr.DataArray, tag_class_map: dict[int, str]
...
def extract_label_dataset(
    ...
    if (actual := feature_labels.dims) != (
        expected := ("strand", "sequence", "feature")
    ):
    ...
    def convert_class_labels_to_biluo(class_labels_input):
        ...
        return xr.DataArray(
            ...
            coords={
                "strand": class_labels_input.strand.values,
                "sequence": class_labels_input.sequence.values,
            },
        )
...
def select_windows(
    ...
    if intergenic_proportion >= 1.0:
        ...
        intergenic_needed = int(
            genic_target * intergenic_proportion / (1 - intergenic_proportion)
        )
    ...
    selected_intergenic = (
        ...
        if intergenic_windows and intergenic_target > 0
        else []
    )
    ...
    selected_genic = (
        ...
        if genic_windows and genic_target > 0
        else []
    )
    ...
    actual_proportion = (
        len(selected_intergenic) / total_selected if total_selected > 0 else 0.0
    )
