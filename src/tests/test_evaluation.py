...
from src.evaluate_gff import evaluate_gff
...
def test_multiple_matches():
    ...
    metrics = ev.evaluate_intervals(pred_intervals, true_intervals)
    assert metrics.tp == 1  # Only one of the (1,5) predictions counts as TP
    assert metrics.fp == 2     # Matched one out of two true intervals
    assert metrics.fn == 1
    
    # Multiple true intervals match single predicted interval
    #TODO what should the behavior actually be here??
    # pred_intervals = [(1, 5), (10, 15)]
    # true_intervals = [(1, 5), (1, 5), (20, 25)]
    # metrics = evaluate_intervals(pred_intervals, true_intervals)
    # assert metrics["true_positives"] == 2    # The (1, 5) prediction matches to both true intervals
    # assert metrics["false_positives"] == 1     # Both (1, 5) true intervals count as matches
    ...
    metrics = ev.evaluate_intervals(pred_intervals, true_intervals)
    assert metrics.tp == 3  # The (1,5) prediction is a TP for both true (1,5) intervals
    assert metrics.fp == 1     # All true intervals are matched except (20, 25)
    assert metrics.fn == 1
...
def test_get_longest_feature():
    ...
    transcript_b.sub_features = [SeqFeature(FeatureLocation(30, 60, strand=1), type="five_prime_UTR"),
    ...
    transcript_a.sub_features = [SeqFeature(FeatureLocation(50, 80, strand=1), type="five_prime_UTR"),
                                 SeqFeature(FeatureLocation(80, 90, strand=1), type="CDS"),
    ...
    transcript_c.sub_features = [SeqFeature(FeatureLocation(10, 20, strand=1), type="five_prime_UTR"),
...
def test_overlap_set_empty():
    ...
    pindices, tindices, pnext, tnext = ev.get_next_overlap_set(
        ...
        [SeqFeature(FeatureLocation(0, 10, strand=1), type="gene")], 1, 1)
...
def test_overlap_set_singletons():
    p_features = [SeqFeature(FeatureLocation(20, 50, strand=1), type="gene"),
                    SeqFeature(FeatureLocation(100, 150, strand=1), type="gene")]
    t_features = [SeqFeature(FeatureLocation(60, 120, strand=1), type="gene")]

    # simple no overlap
    pindices, tindices, pnext, tnext = ev.get_next_overlap_set(p_features, t_features, 0, 0)
    assert pindices == [0]
    assert tindices == []
    assert pnext == 1
    assert tnext == 0
    ...
    # simple no overlap
    pindices, tindices, pnext, tnext = ev.get_next_overlap_set(p_features, t_features, 0, 0, -1)
    assert pindices == [0]
    assert tindices == []
    assert pnext == 1
    assert tnext == 0
    ...
    p_features = [SeqFeature(FeatureLocation(100, 130, strand=1), type="ncRNA"),
    ...
    p_features = [SeqFeature(FeatureLocation(20, 70, strand=1), type="gene"),
                    SeqFeature(FeatureLocation(100, 150, strand=1), type="gene")]
    t_features = [SeqFeature(FeatureLocation(60, 120, strand=-1), type="gene")]
...
def test_overlap_set():
    p_features = [SeqFeature(FeatureLocation(20, 70, strand=1), type="ncRNA"),
                  SeqFeature(FeatureLocation(100, 150, strand=1), type="gene"),
    ...
    t_features = [SeqFeature(FeatureLocation(30, 80, strand=1), type="ncRNA"),
    ...
    pindices, tindices, pnext, tnext = ev.get_next_overlap_set(p_features, t_features, 5, 6)
    assert pindices == [5]
    assert tindices == [6]
    assert pnext == 6
    assert tnext == 7
    ...
    pindices, tindices, pnext, tnext = ev.get_next_overlap_set(p_features, t_features, 5, 6, -1)
    assert pindices == [7]
    assert tindices == [6]
    assert pnext == 8
    assert tnext == 7
...
def test_overlap_set_nested():
    p_features = [SeqFeature(FeatureLocation(30, 80, strand=1), type="gene"),
    ...
    t_features = [SeqFeature(FeatureLocation(20, 70, strand=1), type="gene"),
    ...
    pindices, tindices, pnext, tnext = ev.get_next_overlap_set(p_features, t_features, 6, 5)
    assert pindices == [6, 6]
    assert tindices == [6, 7]
    assert pnext == 7
    assert tnext == 8
...
def test_transcript_to_intervals():
    ...
    transcript.sub_features = [SeqFeature(FeatureLocation(30, 60, strand=1), type="five_prime_UTR"),
                               SeqFeature(FeatureLocation(60, 70, strand=1), type="CDS"),
    ...
    # with UTRs
    assert intervals.cds == P.closedopen(60, 70) | P.closedopen(80, 90) | P.closedopen(100, 110)
    assert intervals.utr == P.closedopen(30, 60) | P.closedopen(110, 130) | P.closedopen(140, 160)
    assert intervals.intron == P.closedopen(70, 80) | P.closedopen(90, 100) | P.closedopen(130, 140)
    assert intervals.intron_cds == P.closedopen(70, 80) | P.closedopen(90, 100)
    assert intervals.exon == P.closedopen(30, 70) | P.closedopen(80, 90) | P.closedopen(100, 130) | P.closedopen(140, 160)

def test_transcript_to_intervals_missing_utr():
    # without UTRs

    transcript = SeqFeature(FeatureLocation(60, 110, strand=1), type="mRNA")
    transcript.sub_features = [SeqFeature(FeatureLocation(60, 70, strand=1), type="CDS"),
                               SeqFeature(FeatureLocation(60, 70, strand=1), type="exon"),
                               SeqFeature(FeatureLocation(80, 90, strand=1), type="CDS"),
                               SeqFeature(FeatureLocation(90, 100, strand=1), type="intron"),
                               SeqFeature(FeatureLocation(100, 110, strand=1), type="CDS")]
    ...
    assert intervals.intron_cds == P.closedopen(70, 80) | P.closedopen(90, 100)
    assert intervals.exon == P.closedopen(60, 70) | P.closedopen(80, 90) | P.closedopen(100, 110)

def test_interval_to_tuples():
    empty_interval = P.empty()
    singleton_interval = P.closedopen(3, 104)
    complex_interval = P.closedopen(33, 155) | P.closedopen(155, 190) | P.closedopen(589, 2509)
...
def test_gene_match():
    true_gene = ev.Intervals(
        ...
        P.closedopen(50, 60),
        P.closedopen(10, 30) | P.closedopen(40, 50) | P.closedopen(60, 90) | P.closedopen(110, 130)
    )
    match_w_tolerance = ev.Intervals(
        ...
        P.closedopen(30, 40) | P.closedopen(50, 60) | P.closedopen(90, 110),
        P.closedopen(50, 60),
        P.closedopen(15, 30) | P.closedopen(40, 50) | P.closedopen(60, 90) | P.closedopen(110, 138)
    )
    cds_match = ev.Intervals(
        P.closedopen(40, 50) | P.closedopen(60, 80),
        P.closedopen(28, 30) | P.closedopen(85, 92) | P.closedopen(110, 138),
        P.closedopen(30, 40) | P.closedopen(50, 60) | P.closedopen(80, 85) | P.closedopen(92, 110),
        P.closedopen(50, 60),
        P.closedopen(28, 30) | P.closedopen(40, 50) | P.closedopen(60, 80) | P.closedopen(85, 92) | P.closedopen(110, 138)
    )
    ...
    assert ev.do_genes_match(match_w_tolerance, true_gene, 10) == (True, True, True)
    assert ev.do_genes_match(match_w_tolerance, true_gene, 5) == (True, True, False)
    assert ev.do_genes_match(cds_match, true_gene, 10) == (True, False, False)
    assert ev.do_genes_match(nonmatch, true_gene, 10) == (False, False, False)
