...
def _validate_semi_markov_inputs(
    ...
    logger.info(
        f"Using inferred max duration in Semi-Markov decoding of {max_duration}"
    )
    ...
    for state, probs in duration_probs.items():
        ...
        if np.any(probs_array < 0) or np.any(probs_array > 1):
            raise ValueError(
                f"Duration probabilities for state {state} must be between 0 and 1"
            )
...
def semi_markov_viterbi_decode(
    ...
    emission_probs, transition_matrix, initial_probs, log_duration, max_duration = (
    ...
    T, N = emission_probs.shape
    logger.info(
        f"Starting Semi-Markov Viterbi decode: T={T:,} positions, N={N} states, max_duration={max_duration}"
    )
...
def _semi_markov_viterbi_decode(
    ...
    for j in range(N):
        ...
        for t in range(1, T):
            cumulative_emission[t, j] = (
                cumulative_emission[t - 1, j] + log_emission[t, j]
            )
        ...
        for j in range(N):
            ...
            for d in range(1, max_dur + 1):
                ...
                if start_time == 0:
                    ...
                    emission_score = (
                        cumulative_emission[t, j]
                        - cumulative_emission[start_time - 1, j]
                    )
...
def brute_force_semi_markov_decode(
    ...
    for segmentation in segmentations:
        score = _score_segmentation(
            segmentation, log_emission, log_transition, log_initial, log_duration
        )
        if score > best_score:
            best_score = score
...
def _generate_all_segmentations(
    T: int, N: int, max_duration: int
...
def _process_region_for_segmentation(args):
    ...
    return (
        start,
        stop,
        _semi_markov_viterbi_decode(
            log_region_emissions,
            log_transition,
            log_initial,
            log_duration,
            max_duration,
...
def semi_markov_viterbi_segmentation(
    ...
    if not 0 <= background_class < N:
        raise ValueError(
            f"background_class must be in [0, {N - 1}], got {background_class}"
        )
    if background_class_buffer >= background_class_min_length // 2:
        raise ValueError(
            f"background_class_buffer ({background_class_buffer}) too large relative to background_class_min_length ({background_class_min_length})"
        )

    logger.info(
        f"Starting segmentation-based Semi-Markov Viterbi: T={T:,}, background_class={background_class}, min_length={background_class_min_length:,}, buffer={background_class_buffer}"
    )
    ...
    if not np.any(valid_mask):
        ...
        logger.info(
            "No sufficiently long background intervals found, processing entire sequence"
        )
        log_emission = _safe_log_probabilities(emission_probs, epsilon)
        return _semi_markov_viterbi_decode(
            log_emission, log_transition, log_initial, log_duration, max_duration
    ...
    logger.info(
        f"Created {len(complex_regions)} complex regions for Semi-Markov decoding"
    )
    ...
    if num_workers > 0:
        ...
        region_results = [
            _process_region_for_segmentation(args) for args in process_args
        ]
